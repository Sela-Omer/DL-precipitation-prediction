...
class SimpleNNSkipConnectionScript(SimpleNNScript, ABC):
    """
    A script for training a simple neural network model with a skip connection on meteorological data.
    ...
    def create_architecture(self, datamodule: pl.LightningDataModule):
        ...
        model = SimpleNN_SkipConnection(self.service, example_input_array=example_input_array, **model_hyperparams)
